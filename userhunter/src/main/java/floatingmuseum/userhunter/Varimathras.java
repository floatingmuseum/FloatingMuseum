...
import floatingmuseum.userhunter.utils.TimeUtil;
...
public class Varimathras {

    private static Varimathras varimathras;
...
//        filter.addAction(Intent.ACTION_USER_PRESENT);//解锁
    ...
    public static Varimathras getInstance() {
        if (varimathras == null) {
            synchronized (Varimathras.class) {
                if (varimathras == null) {
                    varimathras = new Varimathras();
                }
            }
        }
        return varimathras;
    ...
    public void countingAppUsingTime(String newPackageName, String newClassName) {
//        if (event == null) {
//            return;
//        }
...
//            }
//        }
        if (TextUtils.isEmpty(newPackageName) || TextUtils.isEmpty(newClassName)) {
            return;
        }
//        Logger.d("AppTimeUsingManager...当前包名(捕捉所有):" + csPackageName + "...类名:" + event.getClassName());

        if (!isSamePackageName(newPackageName) && hasActivity(newPackageName, newClassName)) {
            packageNameChanged(newPackageName);
        }
    }

    private boolean isSamePackageName(String newPackageName) {
        String currentPackageName = currentAppTimeUsingInfo == null ? "" : currentAppTimeUsingInfo.getPackageName();
        return newPackageName.equals(currentPackageName);
    }
